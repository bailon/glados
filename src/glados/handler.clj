(ns glados.handler
  (:require [compojure.core :refer [defroutes routes wrap-routes]]
            [glados.routes.home :refer [home-routes]]
            
            [glados.middleware :as middleware]
            [glados.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "glados.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (session/start-cleanup-job!)
  (timbre/info "\n-=[ glados started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "glados is shutting down...")
  (stop-nrepl)
